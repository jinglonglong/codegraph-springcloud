#!/usr/bin/env bash
#
# Build a self-contained Springgraph bundle: an official Node runtime + the
# compiled app + its production deps, so Springgraph runs with NO system Node and
...
#
# Output:
#   unix:    release/springgraph-<target>.tar.gz   (launcher: bin/springgraph)
#   windows: release/springgraph-<target>.zip      (launcher: bin/springgraph.cmd)
set -euo pipefail
...
if [ "$OSFAM" = "win32" ]; then
  ...
  printf '@"%%~dp0..\\node.exe" --liftoff-only "%%~dp0..\\lib\\dist\\bin\\springgraph.js" %%*\r\n' \
    > "$STAGE/bin/springgraph.cmd"
else
  cp "$NODE_BIN" "$STAGE/node"
  cat > "$STAGE/bin/springgraph" <<'LAUNCH'
#!/bin/sh
# Resolve symlinks (e.g. the ~/.local/bin/springgraph link install.sh creates) so
...
exec "$DIR/node" --liftoff-only "$DIR/lib/dist/bin/springgraph.js" "$@"
LAUNCH
  chmod +x "$STAGE/bin/springgraph"
fi

# 5. Archive (.zip for Windows, .tar.gz otherwise).
mkdir -p "$OUT"
if [ "$OSFAM" = "win32" ]; then
  ARCHIVE="$OUT/springgraph-${TARGET}.zip"
  rm -f "$ARCHIVE"
  ( cd "$WORK" && zip -rqX "$ARCHIVE" "springgraph-${TARGET}" )
else
  ARCHIVE="$OUT/springgraph-${TARGET}.tar.gz"
  # --no-xattrs: don't embed macOS xattrs that make GNU tar warn on Linux.
  tar --no-xattrs -czf "$ARCHIVE" -C "$WORK" "springgraph-${TARGET}"
fi
